...
class PayOrder(BaseModel, db.Model):
    ...
    member_id = db.Column(db.Integer, db.ForeignKey('tbl_member.id'), nullable=False)# 会员id
...
class PayOrderItem(BaseModel,db.Model):
    ...
    member_id = db.Column(db.Integer, db.ForeignKey('tbl_member.id'), nullable=False)# 会员表
